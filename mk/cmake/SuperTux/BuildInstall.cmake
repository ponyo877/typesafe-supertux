...
if(EMSCRIPTEN)
  ...
  foreach(script jev-controller.js stats.js headstone.png og.png laya-prompt.js laya-table.js laya-rich-prompt.js laya-rich-table.js llm-table.js rl-table.js rl2-table.js coevo-table.js coevo2-table.js coevo3-table.js coevo4-table.js coevo5-table.js)
  ...
  if(SINGLE_LEVEL_BUILD)
    # A start page to pick the mode (index.html), the game (play.html), and
    # the credits and license texts it links to. Serve the build directory.
    configure_file(${CMAKE_CURRENT_SOURCE_DIR}/mk/emscripten/index.html.in ${CMAKE_CURRENT_BINARY_DIR}/index.html)
    configure_file(${CMAKE_CURRENT_SOURCE_DIR}/mk/emscripten/credits.html.in ${CMAKE_CURRENT_BINARY_DIR}/credits.html)
    configure_file(${CMAKE_CURRENT_SOURCE_DIR}/mk/emscripten/template.html.in ${CMAKE_CURRENT_BINARY_DIR}/play.html)
    configure_file(${CMAKE_CURRENT_SOURCE_DIR}/LICENSE.txt ${CMAKE_CURRENT_BINARY_DIR}/LICENSE.txt COPYONLY)
    configure_file(${CMAKE_CURRENT_SOURCE_DIR}/data/AUTHORS ${CMAKE_CURRENT_BINARY_DIR}/AUTHORS.txt COPYONLY)
    foreach(text supertux-credits.txt third-party-licenses.txt)
      configure_file(${CMAKE_CURRENT_SOURCE_DIR}/mk/emscripten/${text} ${CMAKE_CURRENT_BINARY_DIR}/${text} COPYONLY)
    endforeach()
  endif()
